...
@section('content')
            ...
            <div class=" col " bis_skin_checked="1">
                <div class="card" id="vehicleInspection" bis_skin_checked="1">
                    <div class="card-header bg-transparent" bis_skin_checked="1">
                        <h3 class="mb-0">Create Inspection</h3>
                    </div>
                    {!! Form::open(['route' => 'inspection.store', 'method' => 'POST', 'role' => 'form','id'=>'inspectionForm','name'=>'inspectionForm']) !!}
                    <div class="card-body" bis_skin_checked="1">
                            <div><h2>{{$leased->name}}</h2></div>
                            <input type="hidden" name="leasedId" value="{{$leased->id}}">
                            <div class="alert alert-warning hide" id="warning" role="alert"></div>
                            <div class="" bis_skin_checked="1">
                                <div class="container-fluid">
                                    <!-- Begin Modal Content -->
                                    <div class="panel-group" id="inspectionAccordion" role="tablist" aria-multiselectable="true">
                                                    ...
                                                    <div class="panel-body" style="position: relative;">
                                                                            ...
                                                                            <td>
                                                                                <div class="col-md-12">
                                                                                    <div class="form-group">
                                                                                        <input type="text" name="option_{{$item->id}}" placeholder="Add info" class="form-control form-control-alternative" />
                                                                                    </div>
                                                                                </div>
                                                                            </td>
                                                                        @endforeach
                                                                ...
                                                                <div class="col-sm-4 col-md-6">
                                                                    <div class="condition-background-select">
                                                                        <select name="condition-background" id="conditionBackground" tabindex="-1" aria-hidden="true" @if(isset($is_deliver)){{ 'disabled' }}@endif>
                                                                            ...
                                                                            <option value="9" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Van</option>
                                                                            <option value="10" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Mini van</option>
                                                                            <option value="11" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Moto</option>
                                                                            <option value="12" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Caja seca</option>
                                                                            <option value="13" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Camion</option>
                                                                            <option value="14" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Trailer</option>
                                                                            <option value="15" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Montacarga</option>
                                                                            <option value="16" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">Pipa</option>
                                                                            <option value="17" data-img="{{ asset('assets')}}/img/trailers/sandbox.jpg">4X4</option>
                                                                        </select>
                                                                    </div>
                                                                </div>
                                                                @if(!isset($is_deliver))
                                                                    <div class="col-sm-8 col-md-6">
                                                                        <div class="condition-buttons">
                                                                            ...
                                                                            <button type="button" data-type="eraser" class="eraser"><span class="glyphicon glyphicon-erase"></span></button>
                                                                        </div>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                            <canvas width="760" height="416" class="condition-canvas"></canvas>
                                                            <input type="text" hidden name="condition-data" id="conditionData" value="@if(isset($inspection_items[39])){{ $inspection_items[39] }}@endif"
                                                                   original="@if(isset($inspection_items[39])){{ $inspection_items[39] }}@endif" @if(isset($is_deliver)){{ 'disabled' }}@endif>
                                                        @endif

                                                        @if($catJSON->type == 'base64')
                                                            <div class="signature-wrapper-{{ $category->id }}">
                                                                @if($category->id == 7 && isset($inspection_items[40]) || $category->id == 10 && isset($inspection_items[74]))
                                                                    <input type="hidden" name="modified" value="false" @if(isset($is_deliver)){{ 'disabled' }}@endif>
                                                                    <img class="center-block" src="@if($category->id == 7){{ $inspection_items[40] }}@elseif($category->id == 10){{ $inspection_items[74] }}@endif" alt="signature">
                                                                @endif
                                                                <div class="signature-pad @if($category->id == 7 && isset($inspection_items[40]) || $category->id == 10 && isset($inspection_items[74])){{ 'hidden' }}@endif">
                                                        ...
                                                        @if($catJSON->type == 'text-area')
                                                            <div class="k-form-group">
                                                                <textarea class="form-control form-control-alternative" name="commentInspection" id="commentInspection" rows="3" placeholder="Write a comment here ...">{{ $inspection_items[75] ?? null }}</textarea>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
    ...
    @push('js')
        ...
        <script>
            $('form[name=inspectionForm]').submit(async function (e)
            {
                e.preventDefault();
