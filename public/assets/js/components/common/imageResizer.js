...
ImageUploader.prototype.handleFileList = function(fileArray) {
    ...
    if(fileArray.length > This.config.maxFiles) {
        $.alert({
            ...
            type: 'red',
            title: 'Número máximo de imágenes superado.',
            content: "Por favor sube un máximo de " + This.config.maxFiles + " imágenes.",
            buttons: {
                confirm: {
                    text: 'ENTERADO',
                    action: function () {
                        $('.photo-preview.upload').html(This.config.defaultHtml);
                    }
                }
            }
        });
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.success == true){
                ImageUploader.uploads.push(data.picture)
            } else {
                This.progressObject.errors.push(data.msg)
            }
        }
    });
...
ImageUploader.prototype.setConfig = function(customConfig) {
    ...
    this.config.defaultHtml = $('.photo-preview.upload').html();
